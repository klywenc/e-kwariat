...
// Możesz chcieć zaimportować ikonę koszyka, jeśli chcesz jej użyć na przycisku
...
export default function ProductCard({ product }) {
  ...
  if (typProduktu?.nazwa === 'Książka' && product.daneKsiazki) {
    ...
    const { przedmiot, poziomEdukacji, klasa, rokSzkolny, wydawnictwo } = product.danePodrecznika;
    specificDetails = (
      <>
        ...
        {rokSzkolny && <p className="text-xs text-gray-500">Rok szk.: {rokSzkolny}</p>}
        {wydawnictwo && <p className="text-xs text-gray-500">Wyd.: {wydawnictwo}</p>}
      </>
    );
    ...
    // Można by też pobrać autorów/gatunki z powiązanej DaneKsiazki, jeśli taka relacja istnieje
    specificDetails = (
      <>
        ...
        {/* Tutaj można by dodać autora i gatunek, jeśli są dostępne np. przez product.daneKsiazki */}
    ...
    // Dla Outletu możemy chcieć pokazać opisStanu bardziej prominentnie
    // i ewentualnie oryginalny typ produktu, jeśli jest dostępny
    specificDetails = (
      <>
        ...
        {/* Można dodać więcej info, jeśli Outlet ma specyficzne dane */}
      </>
    );
  } else if (typProduktu?.nazwa === 'Pozostałe') {
    // Dla "Pozostałe" możemy nie mieć wielu specyficznych danych
    // Można wyświetlić ogólny opis produktu, jeśli jest
    specificDetails = (
      <>
  ...
  // Domyślnie, jeśli typ nie jest obsłużony, specificDetails pozostanie null
