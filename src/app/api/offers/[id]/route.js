...
export async function PUT(request, { params }) {
    try {
        ...
        // Walidacja ID i podstawowych pól - bez zmian
        ...
        // --- Twoja istniejąca logika `upsert` dla danych specyficznych (bez zmian) ---
        // if (typProduktuId === TYP_ID_KSIAZKA && data.daneKsiazki) { ... }
        // ... (cała Twoja logika `else if` dla danych specyficznych z `upsert`) ...
        if (typProduktuId === TYP_ID_KSIAZKA && data.daneKsiazki) {
        ...
        // ZMIANA: Logika aktualizacji zdjęć
        // Podejście "usuń stare, dodaj nowe"
        // Zakładamy, że `data.zdjecia` to pełna, aktualna lista zdjęć dla produktu
        if (data.zdjecia !== undefined) { // Sprawdzamy czy pole 'zdjecia' przyszło (nawet jeśli jest pustą tablicą)
            await prisma.zdjecieProduktu.deleteMany({
                where: { produktId: produktIdInt },
            });

            if (Array.isArray(data.zdjecia) && data.zdjecia.length > 0) {
                produktUpdateData.zdjecia = {
        ...
        } // Jeśli `data.zdjecia` jest undefined, nie ruszamy istniejących zdjęć.
        ...
        const updatedProduct = await prisma.produkt.update({
            ...
            data: {
                ...
                // `produktUpdateData.zdjecia` jest już ustawione powyżej, jeśli trzeba
            },
