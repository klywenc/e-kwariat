...
// WAŻNE: Te wartości MUSZĄ odpowiadać ID w Twojej tabeli TypProduktu po seedowaniu/stworzeniu.
// W idealnym świecie pobierałbyś te ID z bazy na podstawie unikalnej nazwy/slug'a typu.
  ...
  try {
    const products = await prisma.produkt.findMany({
      include: {
        ...
        // Opcjonalnie: dołącz podstawowe dane dla szybkiego podglądu na liście
        // daneKsiazki: { select: { rokWydania: true } },
      },
...
export async function POST(request) {
  try {
    ...
    // Logika dla każdego typu produktu
    if (typProduktuId === TYP_ID_KSIAZKA && data.daneKsiazki) {
      ...
      // Dla Outlet i Pozostałe nie tworzymy dodatkowych danych specyficznych,
      // chyba że zdecydujesz inaczej i dodasz dla nich modele Dane...
    } else if (Object.keys(data).some(key => key.startsWith('dane'))) {
        // Jeśli typProduktuId nie pasuje, a są jakieś dane specyficzne, to błąd
    ...
    const newProduct = await prisma.produkt.create({
      data: {
        ...
        zdjecia: data.zdjeciaUrls ? {
          create: data.zdjeciaUrls.map((url, index) => ({ url: url, czyGlowne: index === 0, opisAlt: data.zdjeciaAlts?.[index] || `Zdjęcie ${index + 1}` }))
        } : undefined,
      },
      include: {
